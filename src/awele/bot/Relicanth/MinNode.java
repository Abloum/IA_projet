 ...
 */
public class MinNode extends RelicanthNode
{
    /**
     * Constructeur pour un noeud initial
     * @param board La situation de jeu pour laquelle il faut prendre une décision
     */
    MinNode (Board board, int budget)
    {
        this (board, 0, -Double.MAX_VALUE, Double.MAX_VALUE, budget);
    }
     ...
     */
    MinNode (Board board, int depth, double alpha, double beta, int budget)
    {
        super (board, depth, alpha, beta, budget);
    }
     ...
     */
    @Override
    protected RelicanthNode getNextNode (Board board, int depth, double alpha, double beta, int budget)
    {
        return new MaxNode(board, depth, alpha, beta, budget);
    }
