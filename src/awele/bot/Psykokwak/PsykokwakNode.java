 ...
 */
public abstract class PsykokwakNode
{
     ...
     */
    public PsykokwakNode (Board board, int depth, double alpha, double beta)
    {
            ...
            if (board.getPlayerHoles () [i] != 0)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (depth < PsykokwakNode.maxDepth)
                        {
                            /* On construit le noeud suivant */
                            PsykokwakNode child = this.getNextNode (copy, depth + 1, alpha, beta);
    ...
    {
        PsykokwakNode.maxDepth = maxDepth;
        PsykokwakNode.player = board.getCurrentPlayer ();
    }

    private int diffScore (Board board)
    {
        return board.getScore (PsykokwakNode.player) - board.getScore (Board.otherPlayer (PsykokwakNode.player));
    }
    ...
    private double BourrinosShiny(int i, Board board){
            try {
                ...
                if (board.getCurrentPlayer()==PsykokwakNode.player) {
     ...
     */
    protected abstract PsykokwakNode getNextNode (Board board, int depth, double alpha, double beta);
